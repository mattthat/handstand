import css from '../../css/ui-elements/HandstandList.css';
...
export default class HandstandList extends HandstandCustomElement {
    ...
    addItem(item, opts) {
        ...
        if (course.fire) {
            if (typeof this.conditions
                .events.onItemAdded === 'function')
                this.conditions.events.onItemAdded.call(this);
            if (typeof this.conditions
                .events.onListChanged === 'function')
        ...
        if (typeof term === 'string' && this.items) {
            this.conditions.properties.items.filter((item) => {
                if (item.template === term) {
                    this.ul.children.item(
                        this.conditions.properties
                            .items.indexOf(item)).remove();
                    this.conditions.properties
                       .items.splice(this.conditions.properties
            ...
            this.ul.children.item(
                this.conditions.properties
                    .items.indexOf(term)).remove();
            this.conditions.properties
                .items.splice(this.conditions.properties
                    .items.indexOf(term), 1);
        }
        if (typeof this.conditions
            .events.onItemRemoved === 'function')
    ...
    clearItems() {
        ...
        Array.from(this.ul.children).map((el) => { el.remove(); });
        if (typeof this.conditions
            .events.onListChanged === 'function')
            this.conditions.events.onListChanged.call(this);
        return this;
